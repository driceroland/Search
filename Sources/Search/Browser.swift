...
final class Browser: NSObject, ObservableObject {
    @Published private(set) var tabs: [Tab] = [] {
        didSet { if prefs.mruSwitcher { tabSwitcher.tabsChanged(eligible: tabs.map(\.id)) } }
    }
    ...
    @Published var activeID: Tab.ID? {
        didSet {
            ...
            let old = oldValue.flatMap { old in tabs.first { $0.id == old } }
            old?.touch()
            // The switcher (Settings › Tabs) keeps its order and pictures
            // only while it is on; off, a switch records nothing.
    ...
    func close(_ tab: Tab) {
        ...
        if tab.pin != nil {
            if prefs.mruSwitcher { tabSwitcher.forgetPreview(of: tab.id) }
            tab.rest()
