...
`⌃Tab` and `⌃⇧Tab` walk along the row of tabs; `Tab` stays the page's, for moving through a form. `esc` puts away whatever is open. In a peek at a link, `⌘↩` keeps it as a tab.
